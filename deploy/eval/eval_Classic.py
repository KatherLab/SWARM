...
def CalculatePatientWiseAUC(resultCSVPath, uniquePatients, target_labelDict, resultFolder, counter, clamMil = False):
    ...
    print(os.path.join(resultFolder, 'TEST_RESULT_PATIENT_SCORES_' + str(counter) + '.csv'))
    df.to_csv(resultCSVPath+ 'TEST_RESULT_PATIENT_SCORES_' + str(counter) + '.csv', index = False)
